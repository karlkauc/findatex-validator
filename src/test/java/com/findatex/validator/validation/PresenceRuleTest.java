...
class PresenceRuleTest {
    ...
    private ValidationContext ctx(TptFile f, Set<ProfileKey> profiles) {
    ...
    void presenceRuleSkipsRowsOutsideCicScope() {
        ...
        PresenceRule rule = new PresenceRule(spec, TptProfiles.SOLVENCY_II);
        ...
        if (field32.flag(TptProfiles.SOLVENCY_II) == com.findatex.validator.spec.Flag.M) {
            PresenceRule r2 = new PresenceRule(field32, TptProfiles.SOLVENCY_II);
            // Equity row → CIC 3 → field 32 is not in CIC 3 → presence rule must skip.
            assertThat(r2.evaluate(ctx(equityRow, Set.of(TptProfiles.SOLVENCY_II)))).isEmpty();
        }
    ...
    void presenceRuleSilentWhenProfileNotActive() {
        ...
        assertThat(rule.evaluate(ctx(f, Set.of(TptProfiles.NW_675)))).isEmpty();
        // Active profile includes SOLVENCY_II → fires.
        assertThat(rule.evaluate(ctx(f, Set.of(TptProfiles.SOLVENCY_II)))).isNotEmpty();
    }
    ...
    void conditionalPresenceRuleSkipsWhenCicMissing() {
        ...
        ConditionalPresenceRule rule = new ConditionalPresenceRule(field13, TptProfiles.SOLVENCY_II);
        ...
        assertThat(rule.evaluate(ctx(noCic, Set.of(TptProfiles.SOLVENCY_II)))).isEmpty();
    }
    ...
    void conditionalPresenceRuleFiresWhenCicMatchesAndValueMissing() {
        ...
        List<Finding> findings = rule.evaluate(ctx(equity, Set.of(TptProfiles.SOLVENCY_II)));
    ...
    void conditionalPresenceRuleSkipsForFieldsAppliesToAllCic() {
        ...
        ConditionalPresenceRule rule = new ConditionalPresenceRule(field12, TptProfiles.SOLVENCY_II);
        TptFile f = new TestFileBuilder().row(values("17", "x")).build();
        assertThat(rule.evaluate(ctx(f, Set.of(TptProfiles.SOLVENCY_II)))).isEmpty();
    }
}
