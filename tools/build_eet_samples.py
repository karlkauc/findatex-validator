...
from findatex_realistic_values import (
    OVERRIDES_EET, apply_overrides, share_classes,
)
from findatex_sample_helpers import (
    ROOT, FieldRow, load_spec, profile_flags, value_for, write_scenarios, write_xlsx,
)
...
# --- showcase --------------------------------------------------------------
# The numbered fixtures above only need the SFDR_PERIODIC mandatory set (the
# manifest's primaryFlag column). The showcase declares four reporting
# branches as Y, so it has to carry the mandatory fields of every profile
# EetProfileGate will then activate — otherwise the demo is one long list of
# PRESENCE errors for columns the file never claimed to deliver.
SHOWCASE_PROFILES = ["SFDR_PERIODIC", "SFDR_PRECONTRACT",
                     "MIFID_PRODUCTS", "MIFID_DISTRIBUTORS", "LOOK_THROUGH"]
_FLAGS = profile_flags("EET", "V1.1.3")
_SHOWCASE_M = [n for n, f in _FLAGS.items()
               if any(f.get(p) == "M" for p in SHOWCASE_PROFILES)]
# SFDR conditional block (incl. the Art-9 Paris/decarbonisation pair 80/81)
# plus the PAI block, so the sustainability scenarios have their columns.
SHOWCASE_EXTRA = ["6", "8", "9", "10", "28", "30", "31", "33", "35", "36",
                  "40", "41", "42", "43", "44", "45", "46", "47", "48",
                  "80", "81"] + PAI_BLOCK
SHOWCASE_NUMS = _SHOWCASE_M + [n for n in SHOWCASE_EXTRA if n not in _SHOWCASE_M]
SHOWCASE_HEADERS = [BY_NUM[n].path for n in SHOWCASE_NUMS]
...
def s00_showcase() -> list[dict[str, str]]:
    ...
    Product mix is deliberate: one out-of-scope money-market range (NUM 27 =
    "0", every Art-8/9 field empty), one Article 6 bond range, one Article 8
    equity range and one Article 9 sustainable range, so the SFDR conditional
    rules have something to say on each. The two PAI-considering products
    populate the 27-field PAI block; the others leave it blank, which is what
    NUM 33 = "N" means.
    """
    rows: list[dict[str, str]] = []
    for sc in share_classes(make_isin):
        row = {n: value_for(BY_NUM[n].codif) for n in SHOWCASE_NUMS}
        apply_overrides(row, OVERRIDES_EET, sc)
        if not sc["pai"]:
            # value_for fills every Y/N PAI indicator with "Y"; a product that
            # does not consider PAI reports an empty block instead.
            for n in PAI_BLOCK:
                row[n] = ""
        rows.append(row)
    _apply_showcase_defects(rows)
    return rows


# Row index → {num: value}. Roughly one defect per three rows, spread over the
# rule families a reviewer would actually meet in a real EET delivery.
SHOWCASE_DEFECTS: dict[int, dict[str, str]] = {
    2:  {"15": "31/12/2025"},        # FORMAT/15 — dd/mm/yyyy instead of ISO 8601
    5:  {"26": "EuroX"},             # FORMAT/26 — not an ISO 4217 code
    8:  {"25": ""},                  # PRESENCE/25 — instrument name missing
    11: {"5": "2026-13-40"},         # FORMAT/5  — impossible generation timestamp
    13: {"31": "", "45": ""},        # EET-XF-ART9-MIN-LT / -MIN-ENV on an Art-9 class
    16: {"24": "42"},                # FORMAT/24 — outside the closed list {1..10, 99}
    18: {"581": ""},                 # PRESENCE/581 — sustainability-risk flag missing
    21: {"28": ""},                  # EET-XF-SFDR-OUT-OF-SCOPE — 27="0" needs 28
    23: {"23": ""},                  # PRESENCE/23 — no instrument identifier
    4:  {"41": ""},                  # EET-XF-ART8-MIN-SI — Art-8 class says it holds
                                     #   sustainable investments but reports no minimum
    22: {"30": "0.40"},              # EET-XF-ART30-MUST-BE-ABSENT — out-of-scope
                                     #   product carrying an Art-8 proportion
}


def _apply_showcase_defects(rows: list[dict[str, str]]) -> None:
...
def main() -> int:
    write_xlsx(OUT / "00_showcase.xlsx", MANIFEST["sheetName"], SHOWCASE_HEADERS,
               [[r[n] for n in SHOWCASE_NUMS] for r in s00_showcase()])
