...
describe('ExternalValidationToggle', () => {
  ...
  it('shows only the master toggle when externalEnabled=false', () => {
    ...
    expect(screen.getByText(/Enable GLEIF \/ OpenFIGI online checks/)).toBeInTheDocument();
    // Sub-section labels should not be rendered yet.
    expect(screen.queryByText(/Check LEI against GLEIF/)).not.toBeInTheDocument();
    expect(screen.queryByText(/Check ISIN against OpenFIGI/)).not.toBeInTheDocument();
  });

  it('expands LEI + ISIN sections when externalEnabled=true', () => {
    renderToggle({ externalEnabled: true });
    expect(screen.getByText(/Check LEI against GLEIF/)).toBeInTheDocument();
    expect(screen.getByText(/Check ISIN against OpenFIGI/)).toBeInTheDocument();
  ...
  it('forwards master toggle changes via onExternalEnabledChange', async () => {
    ...
    const masterCheckbox = screen.getByLabelText(/Enable GLEIF \/ OpenFIGI online checks/);
